  ...
  (let ((LOVE (apply #'concat
	...
	(cl-loop repeat 20
		 do (mapc #'(lambda (s) (mapc #'insert (make-list 52 s)) (insert "\n"))
			  '("LOVE" "SISTER" "SIMONE")))
	(cl-flet ((b (fo to) (add-face-text-property fo to '(:foreground "lightcyan")))
		  (p (fo to) (add-face-text-property fo to '(:foreground "lightred")))
		  (w (fo to) (add-face-text-property fo to '(:foreground "white"))))
